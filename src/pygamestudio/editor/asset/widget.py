...
class SortAssetButton(QPushButton):
    ...
    def __show_context_menu(self, pos):
        ...
        for (text, sort_type) in action_text_list:
            ...
            action.setChecked(True) if sort_type == self.__sort_type else action.setChecked(False)
            action.triggered.connect(self.__set_sort_type)
            menu.addAction(action)
            action_group.addAction(action)

        menu.exec(self.mapToGlobal(pos))

    def __set_sort_type(self):
        self.__sort_type = self.sender().data()
        self.sort_signal.emit(self.__sort_type)
